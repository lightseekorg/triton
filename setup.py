    ...
    def build_extension(self, ext):
        ...
        if check_env_flag("TRITON_BUILD_WITH_CLANG_LLD"):
            cmake_args += [
                ...
                "-DCMAKE_MODULE_LINKER_FLAGS=-fuse-ld=lld -Wl,--icf=safe",
                "-DCMAKE_SHARED_LINKER_FLAGS=-fuse-ld=lld -Wl,--icf=safe",
            ]
        ...
        if "TRITON_BUILD_PROTON" not in os.environ:
    ...
    for backend in backends:
        ...
        if backend.language_dir:
            ...
            extra_dir = os.path.abspath(
                os.path.join(os.path.dirname(__file__), "python", "tokenspeed_triton", "language", "extra"))
            for x in os.listdir(backend.language_dir):
        ...
        if backend.tools_dir:
            ...
            # `triton.tools.extra`.
            extra_dir = os.path.abspath(
                os.path.join(os.path.dirname(__file__), "python", "tokenspeed_triton", "tools", "extra"))
            for x in os.listdir(backend.tools_dir):
...
TRITON_VERSION = "3.8.10" + get_triton_version_suffix()
...
setup(
    ...
    package_data={
        ...
        # tokenspeed_triton package root (<build_lib>/tokenspeed_triton/).
        "tokenspeed_triton": [
            "include/**/*.h",
            "include/**/*.h.inc",
            "include/**/*.td",
