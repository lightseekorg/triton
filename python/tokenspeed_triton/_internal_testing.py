...
from tokenspeed_triton.runtime.jit import TensorWrapper, reinterpret, type_canonicalisation_dict
...
def get_current_target():
    if is_interpreter():
        return None
    return tokenspeed_triton.runtime.driver.active.get_current_target()
...
def torch_dtype_name(dtype) -> str:
    if isinstance(dtype, tokenspeed_triton.language.dtype):
        return dtype.name
...
def unwrap_tensor(t: Union[torch.Tensor, tokenspeed_triton.runtime.jit.TensorWrapper]) -> torch.Tensor:
