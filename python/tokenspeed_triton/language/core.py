...
def load(pointer, mask=None, other=None, boundary_check=(), padding_option="", cache_modifier="", eviction_policy="",
    ...
    if padding_option or boundary_check:
        raise ValueError("`padding_option` or `boundary_check` is only supported for block pointers")
...
def store(pointer, value, mask=None, boundary_check=(), cache_modifier="", eviction_policy="", _semantic=None):
    ...
    if mask is not None:
        mask = _semantic.to_tensor(mask)
    if boundary_check:
        raise ValueError("`boundary_check` is only supported for block pointers")
    cache_policy = _normalize_cache_policy(None, cache_modifier, eviction_policy)
