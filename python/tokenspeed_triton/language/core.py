...
def builtin(fn: T) -> T:
    ...
    def wrapper(*args, **kwargs):
        if "_semantic" not in kwargs or kwargs["_semantic"] is None:
            raise ValueError("Did you forget to add @tokenspeed_triton.jit ? "
...
class dtype(base_type):
    ...
    def __repr__(self):
        ...
        return f'tokenspeed_triton.language.{self.codegen_name()}'
...
class tensor(base_value):
    ...
    functions in :py:mod:`tokenspeed_triton.language` operate on and return tensors.

    Most of the named member functions here are duplicates of the free functions
    in :code:`tokenspeed_triton.language`.  For example, :code:`tokenspeed_triton.language.sqrt(x)` is
...
def _wrap_init_args(x):
    if isinstance(x, tuple):
        from tokenspeed_triton.compiler.code_generator import _apply_to_tuple_values
    ...
    class aggregate_value(base_value):
        ...
        def __new__(this_cls, *args, _semantic=None, _generator=None, **kwargs):
            ...
            if isinstance(init, JITCallable):
                # raise ValueError(f"{cls.__name__}.__init__ cannot be a @tokenspeed_triton.jit function")
                pass
            else:
...
def atomic_poll(pointer, expected_value, sem=None, scope=None, timeout_ns=None, _semantic=None):
    ...
    :return: True if the expected value was observed, or False if the timeout
        expired first.
    :rtype: tokenspeed_triton.language.tensor
    """
...
def reduce(input, axis, combine_fn, keep_dims=False, _semantic=None, _generator=None):
    ...
    :param axis: the dimension along which the reduction should be done. If None, reduce all dimensions
    :type axis: int | None
    :param combine_fn: a function to combine two groups of scalar tensors (must be marked with @tokenspeed_triton.jit)
...
def device_print(prefix, *args, hex=False, _semantic=None):
    ...
    .. code-block:: python
        ...
        tokenspeed_triton.runtime.driver.active.utils.set_printf_fifo_size(size_bytes)
...
class static_range(base_value):
    ...
    :note: This is a special iterator used to implement similar semantics to Python's :code:`range` in the context of
        :code:`tokenspeed_triton.jit` functions. In addition, it also guides the compiler to unroll the loop aggressively.
    ...
    def __iter__(self):
        raise RuntimeError("static_range can only be used in @tokenspeed_triton.jit'd functions")
...
class range(base_value):
    ...
    .. code-block:: python
        ...
        def kernel(...):
            for i in tl.range(10, num_stages=3):
                ...
    :note: This is a special iterator used to implement similar semantics to Python's :code:`range` in the context of
        :code:`tokenspeed_triton.jit` functions. In addition, it allows user to pass extra attributes to the compiler.
    :param arg1: the start value.
    :param arg2: the end value.
    ...
    def __iter__(self):
        raise RuntimeError("tl.range can only be used in @tokenspeed_triton.jit'd functions")
...
class condition(base_value):
    ...
    :note: This is a special wrapper used to annotate while loops in the context of
        :code:`tokenspeed_triton.jit` functions. It allows user to pass extra attributes to the compiler.
    :param disable_licm: Tells the compiler it shouldn't hoist loop invariant
        code outside the loop. This is often useful to avoid creating long liveranges
        within a loop.
    """
