...
def _cache_invalidating_environment(environment):
    from tokenspeed_triton._C.libtriton import get_cache_invalidating_env_vars
...
def _load_jit_callable(module_name, name):
    from tokenspeed_triton.runtime.jit import JITCallable
...
class _JITFunctionPickler(CloudPickler):
    ...
    def reducer_override(self, obj):
        from tokenspeed_triton.runtime.jit import BoundConstexprFunction, ConstexprFunction, JITCallable, JITFunction
...
def _preload_with_compile_context(function, name, key, serialized_target, serialized_options, signature, constants,
    ...
    from tokenspeed_triton.compiler import ASTSource, compile
    ...
    if function.is_gluon():
        from tokenspeed_triton.experimental.gluon._runtime import GluonASTSource
...
def _warmup_kernel(kernel, args, grid, kwargs):
    from tokenspeed_triton.runtime.autotuner import Autotuner
    ...
    try:
        with (
            ...
            if trace_directory is not None:
                from tokenspeed_triton._compile_warmup import CompilationTrace
...
class ProcessPoolWarmupDispatcher:
    ...
    def _capture_preloads(self, instrumentation_mode, environment, kernel, args, grid, kwargs, test):
        from tokenspeed_triton.runtime import jit
