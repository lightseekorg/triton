...
def _compile_without_gsan():
    with tokenspeed_triton.knobs.compilation.scope():
...
def warmup_gsan_kernels() -> None:
    ...
    with _compile_without_gsan():
        _synchronize_vector_clocks_kernel.warmup(
            tokenspeed_triton.MockTensor(tl.uint8),
            0,
            0,
            0,
            0,
...
@tokenspeed_triton.jit(do_not_specialize=["stride_bytes", "num_sms", "num_threads", "header_bytes"])
...
def synchronize_launch_stream(device: int) -> None:
    ...
    grid = (tokenspeed_triton.cdiv(layout.num_threads, 128), 1, 1)
    with _compile_without_gsan():
...
def synchronize_process_group_barrier(device: int, peer_devices: tuple[int, ...]) -> None:
    ...
    BLOCK_SIZE = 128
    grid = (tokenspeed_triton.cdiv(local_layout.num_threads, BLOCK_SIZE), 1, 1)
    with _compile_without_gsan():
