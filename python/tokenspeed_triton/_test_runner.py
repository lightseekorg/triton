...
from tokenspeed_triton._compile_warmup import _require_complete_warmup, summarize_compile_trace
from tokenspeed_triton._compile_warmup_pool import SharedWarmupCoordinator
...
def _pytest(*arguments, workers=None, import_mode=None, distribution="worksteal"):
    command = [sys.executable, "-m", "pytest", "-p", "tokenspeed_triton._compile_warmup", "-s", "--tb=short"]
...
def _unit(args):
    ...
    environment.update({
        ...
        "1", "TRITON_DISABLE_LINE_INFO":
        "0", "LLVM_PASS_PLUGIN_PATH":
        "python/tokenspeed_triton/instrumentation/libGPUInstrumentationTestLib.so"
    })
    return _run(
