...
def _discover_backends() -> dict[str, Backend]:
    ...
    # discover only in-tree backends under the `tokenspeed_triton.backends` namespace.
    ...
    if skip_entrypoints_env == "1":
        ...
        for name in os.listdir(root):
            ...
            if name.startswith('__'):
                continue
            compiler = importlib.import_module(f"tokenspeed_triton.backends.{name}.compiler")
            driver = importlib.import_module(f"tokenspeed_triton.backends.{name}.driver")
            backends[name] = Backend(_find_concrete_subclasses(compiler, BaseBackend),
                                     _find_concrete_subclasses(driver, DriverBase))
        return backends

    # Default path: discover via entry points for out-of-tree/downstream plugins.
    for ep in entry_points().select(group="tokenspeed_triton.backends"):
