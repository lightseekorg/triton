...
def _find_compiler(language: str) -> str:
    if language == "c":
        ...
        if cc is not None:
            return cc
        raise RuntimeError(
            "Failed to find C compiler. Please specify via CC environment variable or set tokenspeed_triton.knobs.build.impl.")
    ...
    raise RuntimeError(
        "Failed to find C++ compiler. Please specify via CXX environment variable or set tokenspeed_triton.knobs.build.impl.")
