...
from tokenspeed_triton.runtime.jit import KernelInterface
...
def _patch_lang(fn):
    ...
    assert len(langs) >= 1, "tokenspeed_triton.language must be visible from within jit'd function"
    for lang in langs:
...
def _implicit_cvt(arg):
    ...
    if isinstance(arg, int):
        ty = tl.str_to_ty(tokenspeed_triton.runtime.jit.mangle_type(arg), None)
        dtype = np.int32
        if -2**31 <= arg < 2**31:
            dtype = np.int32
        elif 2**31 <= arg < 2**32:
...
def _unwrap_tensor(t):
    if isinstance(t, tokenspeed_triton.runtime.jit.TensorWrapper):
        return t.base
    return t


def _rewrap_tensor(t, original_tensor):
    if isinstance(original_tensor, tokenspeed_triton.runtime.jit.TensorWrapper):
        return tokenspeed_triton.runtime.jit.TensorWrapper(t, original_tensor.dtype)
    return t
    ...
    def __call__(self, *args_dev, **kwargs):
        ...
        try:
            ...
            try:
                ...
                if tokenspeed_triton.knobs.compilation.front_end_debugging:
                    raise
