...
from tokenspeed_triton import __version__, knobs
...
def triton_key():
    ...
    # compiler
    path_prefixes = [
        (os.path.join(TRITON_PATH, "compiler"), "tokenspeed_triton.compiler."),
        (os.path.join(TRITON_PATH, "backends"), "tokenspeed_triton.backends."),
    ]
    ...
    for lib in pkgutil.walk_packages([language_path], prefix="tokenspeed_triton.language."):
        with open(lib.module_finder.find_spec(lib.name).origin, "rb") as f:
            contents += [hashlib.sha256(f.read()).hexdigest()]
