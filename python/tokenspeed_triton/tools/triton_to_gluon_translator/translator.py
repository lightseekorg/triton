...
import tokenspeed_triton.language as tl  # type: ignore[import-untyped]
...
from tokenspeed_triton.tools.triton_to_gluon_translator.slice_kernel import (
    GlobalValue,
    ReferenceRewriter,
    RewriteFn,
    add_sugar_rewrites,
...
def add_one_to_one_rewrites(rewrites: list[RewriteFn]) -> None:
    import tokenspeed_triton.experimental.gluon.language as gl  # type: ignore[import-untyped]

    for value in vars(gl).values():
        module = inspect.getmodule(value)
        if module is None:
            continue
        if getattr(value, "__triton_builtin__", False) and is_submodule(module, ["tokenspeed_triton.language"]):
...
class Translator(ReferenceRewriter):
    ...
    def __post_init__(self) -> None:
        ...
        self.imports.add("import tokenspeed_triton.experimental.gluon as gluon")
        self.imports.add("import tokenspeed_triton.experimental.gluon.language as gl")
