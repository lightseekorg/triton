...
import tokenspeed_triton.backends
...
will compile tokenspeed_triton.JITFunction of name `kernel` inside the file `/path/to/kernel.py`.
...
def compile_kernel(args: CompileArgs):
    ...
    target = tokenspeed_triton.backends.compiler.GPUTarget(*args.target.split(":")) \
        if args.target else tokenspeed_triton.runtime.driver.active.get_current_target()
    backend = tokenspeed_triton.compiler.make_backend(target)
    kwargs = {"num_warps": args.num_warps, "num_stages": args.num_stages}
    options = backend.parse_options(kwargs)
    ccinfo = tokenspeed_triton.compile(src, target=target, options=options.__dict__)
    ...
    ty_to_cpp = tokenspeed_triton.runtime.driver.active.map_python_to_cpp_type
