...
SmallVector<int32_t> getLaneTileForLayout(const LocalMemOpTile &tile,
                                          ArrayRef<int32_t> lane,
                                          int32_t vecSize, int32_t bitwidth,
                                          int32_t numBanks) {
  ...
  if (!tile.laneAddr.empty()) {
    // laneAddr defines the lane basis indices used to construct the physical
    // address layout of the instruction.
    res = tile.getLaneAddr(lane);
  } else {
    // If neither representation is provided, use the standard assumption for
  ...
  assert(res.size() == lane.size() - log2Phase &&
         "The lane tile should have the expected number of elements");
  return res;
}

SmallVector<int32_t> getLaneTileForBankConflicts(const LocalMemOpTile &tile,
                                                 ArrayRef<int32_t> lane,
                                                 int32_t vecSize,
                                                 int32_t bitwidth,
                                                 int32_t numBanks) {
  if (tile.laneMask.empty())
    return getLaneTileForLayout(tile, lane, vecSize, bitwidth, numBanks);

  auto log2Vec = llvm::Log2_32(std::max<int32_t>(1, (vecSize * bitwidth) / 32));
  auto log2Bank = llvm::Log2_32(numBanks);
  auto log2Phase = std::max<int32_t>(0, log2Vec + lane.size() - log2Bank);
  auto res = tile.getLaneMask(lane);
  assert(res.size() == lane.size() - log2Phase &&
         "The lane mask should have the expected number of elements");
  return res;
}
...
bankConflictsLdSt(const LinearLayout &src, const LinearLayout &dst,
  ...
  auto tileSrc = getLaneTileForBankConflicts(srcTile, srcLane, vecSize,
                                             bitwidth, numBanks);
  auto tileDst = getLaneTileForBankConflicts(dstTile, dstLane, vecSize,
                                             bitwidth, numBanks);
...
LinearLayout optimalSwizzlingLdSt(const LinearLayout &src,
  ...
  auto log2Vec = llvm::Log2_32(std::max<int32_t>(1, (vecSize * bitwidth) / 32));
  auto tileSrc =
      getLaneTileForLayout(srcTile, laneSrc, vecSize, bitwidth, numBanks);
  auto tileDst =
      getLaneTileForLayout(dstTile, laneDst, vecSize, bitwidth, numBanks);
  auto smem =
